...
class VoskSpeechRecognizer : AudioPipeline.SpeechRecognition {
    ...
    fun initialize(modelPath: String): Boolean {
        if (isInitialized) {
            logger.warn("Vosk识别器已经初始化")
            return true
        }
        
        logger.info("初始化Vosk识别器，模型路径: $modelPath")
        
        try {
            // 加载模型
            model = vosk_model_new(modelPath)
            if (model == null) {
                logger.error("无法加载Vosk模型: $modelPath")
                return false
            }
            ...
            isInitialized = true
            logger.info("Vosk识别器初始化成功，默认关键词: $keywords")
            return true
        } catch (e: Exception) {
            logger.error("初始化Vosk识别器失败: ${e.message}")
            cleanupResources()
            return false
        }
    }
    ...
    fun updateKeywords(keywords: String) {
        if (!isInitialized || recognizer == null) {
            logger.warn("Vosk识别器未初始化，无法更新关键词")
            return
        }
        ...
        try {
            ...
            logger.info("成功更新Vosk关键词: $jsonArray")
        } catch (e: Exception) {
            logger.error("更新Vosk关键词时发生异常: ${e.message}")
        }
    }
    ...
    override fun recognize(audio: ByteArray, length: Int): AudioPipeline.SpeechRecognition.RecognitionResult {
        ...
        logger.info("【调试】Vosk识别开始 #${totalCalls}: 音频长度=${length}, 时间=${startTime}")
        
        try {
            // 异常情况检查
            if (audio.isEmpty() || length <= 0) {
                logger.info("【调试】Vosk收到空音频: 长度=${length}")
                if (shouldLogError()) {
            ...
            // 检查识别器状态
            if (recognizer == null) {
                logger.info("【调试】Vosk识别器无效")
                if (shouldLogError()) {
                    logger.error("Vosk识别器无效")
                }
            ...
            try {
                ...
                logger.info("【调试】Vosk处理音频: 大小=${shortData.size}")
                ...
                if (json != null) {
                    // 记录所有JSON结果以便调试
                    logger.info("【调试】Vosk识别原始JSON: $json")
                    ...
                    // 记录诊断
                    if (totalCalls % 100 == 0) {
                        logger.debug("Vosk识别统计: 总调用=${totalCalls}, 成功=${successfulCalls}, " +
                    ...
                    failedCalls++
                    logger.info("【调试】Vosk返回空结果")
                    if (shouldLogError()) {
                        logger.error("Vosk处理音频失败: null结果")
                    }
            ...
            failedCalls++
            logger.info("【调试】Vosk处理异常: ${e.message}")
            if (shouldLogError()) {
                logger.error("Vosk处理异常: ${e.message}")
            }

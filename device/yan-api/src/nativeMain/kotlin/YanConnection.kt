    ...
    fun connect() {
        try {
            ...
            memScoped {
                ...
                println("哈哈哈")
                if (pyIp == null) {
                    println("错误：无法创建 Python 字符串对象")
                    return
                }
                println("创建的 Python 对象地址：${pyIp.rawValue.toLong()}") // 调试输出
                // 显式增加引用计数，防止对象被提前回收
//                val str = PyObject_Str(pyIp)
                println("引用计数")
//                if (str == null) {
//                    println("错误：无法创建 Python 字符串对象")
//                    return
...
//                Py_IncRef(str)
                Py_IncRef(pyIp)
                println("yan_api_init==>>")
                // 调用 yan_api_init，传入 pyIp（类型会自动转换为 CValuesRef<_object>）
                yan_api_init(null) // 根据接口签名传入正确参数
                println("引用计数2")
                println("yan_api_init result: ")
                // 显式释放 Python 对象引用（避免内存泄漏）
//                Py_DecRef(str)
                // 启动状态监控
                monitorDeviceStatus()

...
cpdef  public int get_robot_battery_value():
    """获得机器人电量百分比
...
cpdef  public dict get_robot_fall_management_state():
    """获得机器人摔倒管理状态
...
cpdef  public dict set_robot_fall_management_state(enable: bool):
    """设置机器人摔倒管理开关
...
cpdef  public dict get_robot_language():
    """获取机器人语言
...
cpdef  public dict  set_robot_language(language: str):
    """设置机器人语言
...
cpdef  public str  get_button_led_color_value():
    """返回机器人胸口按钮灯颜色
...
cpdef  public str get_button_led_mode_value():
    """返回机器人胸口按钮灯点亮模式
...
cpdef  public str get_eye_led_color_value():
    """返回机器人眼睛LED颜色
...
cpdef  public str  get_eye_led_mode_value():
    """返回机器人眼睛LED点亮模式
...
cpdef  public dict get_robot_led():
    """获取机器人灯效
...
cpdef  public dict set_robot_led(type: str, color: str, mode: str):
    """设置机器人灯效
...
cpdef public bint sync_set_led(type: str, color: str, mode: str):
    """设置机器人灯效,设置完成后返回
...
cpdef  public str get_robot_version_info_value(type: str):
    """获取机器人版本信息
    Args:
...
cpdef  public dict get_robot_version_info(type: str):
    """获取机器人版本信息
...
cpdef  public dict get_robot_mode():
    """获取机器人运行模式
...
cpdef  public int get_robot_volume_value():
    """获得机器人音量
    Returns:
...
cpdef  public dict get_robot_volume():
    """获得机器人音量
...
cpdef public bint set_robot_volume_value(volume: int):
    """设置机器人音量
...
cpdef  public dict set_robot_volume(volume: int):
    """设置机器人音量
...
cpdef  public dict get_joystick_buttons_list():
    """获得手柄按键信息
...
cpdef  public dict get_joystick_buttons_list_value():
    """获得手柄按键信息结果
...
cpdef  public  dict delete_media_music(name: str):
    """删除音乐文件
...
cpdef  public dict get_media_music_state():
    """获取机器人音乐播放状态
...
cpdef  public dict upload_media_music(filePath: str):
    """上传音乐文件
...
cpdef  dict start_play_music(name: str = ""):
    """播放音乐
...
cpdef  public dict  stop_play_music():
    """停止播放音乐
...
cpdef  public dict get_media_music_list():
    """获取音乐列表
...
cpdef  bint sync_play_music(name: str = ""):
    """播放音乐，播放完成后返回
...
cpdef  public dict delete_motion(name: str):
    """删除动作文件
...
cpdef  public dict get_current_motion_play_state():
    """获得当前动作文件执行状态
...
cpdef  public dict get_current_layer_motion_play_state():
    """获得当前动作文件执行状态
...
def __control_motion_play_state(operation: str = "start", name: str = "reset", direction: str = "",
    ...
    param = {"operation": operation, "motion": {"name": name, "repeat": repeat, "speed": speed}, "timestamp": timestamp,
             "version": version}
    if (len(direction) != 0):
...
cpdef  dict start_play_motion(name: str = "reset", direction: str = "", speed: str = "normal", repeat: int = 1,
...
cpdef  dict pause_play_motion(name: str = "", timestamp: int = 0, version: str = "v1"):
    """暂停动作执行
...
cpdef dict  resume_play_motion(name: str = "", timestamp: int = 0, version: str = "v1"):
    """恢复动作执行
...
cpdef  dict stop_play_motion(name: str = "", timestamp: int = 0, version: str = "v1"):
    """停止动作执行
...
cpdef bint  sync_play_motion(name: str = "reset", direction: str = "", speed: str = "normal", repeat: int = 1,
                         version: str = "v1"):
    """开始执行动作，执行完成后返回
...
cpdef  public dict upload_motion(filePath: str):
    """上传动作文件
...
cpdef  public dict get_motion_list():
    """获取动作文件列表
...
cpdef public dict control_motion_gait_impl(speed_v: int, speed_h: int, steps: int, period: int, wave: bool):
    return control_motion_gait(speed_v, speed_h, steps, period, wave)
cpdef dict control_motion_gait(speed_v: int = 0, speed_h: int = 0, steps: int = 0, period: int = 1, wave: bool = False):
    """机器人步态动作控制
    ...
    param = {"speed_v": speed_v, "speed_h": speed_h, "steps": steps, "period": period, "timestamp": timestamp,
        "wave": wave}
    json_data = json.dumps(param)
    response = requests.put(url=motion_url, data=json_data, headers=headers)
    res = json.loads(str(response.content.decode("utf-8")))
...
cpdef  public  dict  get_motion_gait_state():
    """获取机器人步态执行状态
...
cpdef  public dict exit_motion_gait():
    """退出机器人步态执行
...
cpdef  bint sync_do_motion_gait(speed_v: int = 0, speed_h: int = 0, steps: int = 0, period: int = 1, wave: bool = False):
    """机器人步态动作控制,执行完成后返回
    ...
    return True
####aprilTag
cpdef  public dict get_aprilTag_recognition_status():
    """查询aprilTag 识别状态
...
cpdef  dict start_aprilTag_recognition(tags: List, enableStream: bool = False):
    """开启aprilTag识别
    Args:
...
cpdef  public dict stop_aprilTag_recognition():
    """关闭aprilTag识别
    Returns:
           Dict:
           e.g::
...
PqrStream = None
cpdef  dict start_QR_code_recognition(enableStream: bool = False):
    """开启二维码识别
    Args:
...
cpdef  public dict stop_QR_code_recognition():
    """关闭二维码识别
    Returns:
           Dict:
           e.g::
...
cpdef  dict sync_do_QR_code_recognition(timeOut: int = 8):
    """开启二维码识别,识别到或超时后返回
    Args:
...
####ObjectTracking####
cpdef   public dict get_object_tracking_status():
    """获取物体跟踪工作状态
...
cpdef dict  start_object_tracking(name: str = "", width: int = 0, height: int = 0):
    """开启物体跟踪
    Args:
...
cpdef  public dict  stop_object_tracking():
    """关闭物体跟踪
    Returns:
           e.g::
...
cpdef  dict config_object_tracking(track_timeout: int, detect_timeout: int):
    """配置物体跟踪
    Args:
...
####Servos####
cpdef   public int get_servo_angle_value(name: str):
    """查询舵机角度值
...
cpdef   public dict get_servos_angles(names: List[str]):
    """查询舵机角度值
...
cpdef  dict set_servos_angles(angles: Dict[str, int], runtime: int = 200):
    """设置舵机角度值
...
cpdef   public dict set_servos_angles_layers(data: Dict[str, Dict[int, int]]):
    """设置舵机角度值
...
cpdef  dict sync_set_servo_rotate(angles: Dict[str, int], runtime: int = 200):
    """设置舵机角度值,设置完成后返回
...
cpdef  public dict get_servos_mode(names: List[str]):
    """查询舵机工作模式
...
cpdef   public dict set_servos_mode(mode: str, servos: List[str]):
    """设置舵机工作模式
...
cpdef  public dict sensor_calibration(id: int):
    """传感器校准
...
cpdef   public dict get_sensors_list():
    """获取所有传感器的列表
...
cpdef  public str get_sensors_environment_value():
    """获取环境传感器值(便利方法)
...
cpdef   public str get_sensors_environment():
    """获取环境传感器值
...
cpdef  public dict get_sensors_gyro():
    """获取九轴陀螺仪运动传感器值
...
cpdef  public str get_sensors_infrared_value():
    """获取红外距离传感器值(便利方法)
...
cpdef  dict get_sensors_infrared(id: List[int] = None, slot: List[int] = None):
    """获取红外距离传感器值
...
cpdef  public str get_sensors_pressure_value():
    """读取机器人压力传感器值(便利方法)
...
cpdef  dict get_sensors_pressure(id: List[int] = None, slot: List[int] = None):
    """读取机器人身上的压力传感器值
...
cpdef   public str get_sensors_touch_value():
    """获取触摸传感器值(便利方法)
...
cpdef   public str get_sensors_ultrasonic_value():
    """获取超声传感器值(便利方法)
...
cpdef  dict get_sensors_ultrasonic(id=None, slot=None):
    """获取超声传感器值
...
cpdef  public dict stop_voice_asr():
    """停止语音识别服务
...
cpdef  public dict get_voice_asr_state():
    """获取语义理解工作状态
...
cpdef public dict start_voice_asr_impl(continues: bool, timestamp: int):
    return start_voice_asr(continues, timestamp)

cpdef dict  start_voice_asr(continues=False, timestamp=0):
    """开始语义理解
...
cpdef  public dict sync_do_voice_asr_value():
...
cpdef   public dict sync_do_voice_asr():
    """执行一次语义理解并获得返回结果
...
cpdef  public dict delete_voice_asr_offline_syntax(grammar: str):
    """删除指定离线语法名称下的所有配置
...
cpdef   public dict get_voice_asr_offline_syntax(grammar: str):
    """获取指定语法名称下的所有配置
...
cpdef  public dict create_voice_asr_offline_syntax(object: Dict):
    """创建一个新的离线语法名称配置
...
cpdef  public dict update_voice_asr_offline_syntax(object: Dict):
...
cpdef   public dict get_voice_asr_offline_syntax_grammars():
    """获取所有离线语法名称
...
cpdef  public start_voice_tts_impl(tts: str , interrupt: bool , timestamp: int):
    return start_voice_tts(tts,interrupt,timestamp)
...
def __control_visual_task(option: str, type: str, operation: str = "start", timestamp: int = 0):
    ...
    """
    visions_url = basic_url + "visions"
    param = {"option": option, "type": type, "operation": operation, "timestamp": timestamp}
    json_data = json.dumps(param)
...
def gamepad_key_to_string(GamepadKey key):
    if key == GamepadKey.L1:
        ...
        return "key_L2"
    elif key == GamepadKey.R1_:
        return "key_R1"
    elif key == GamepadKey.R2_:
        return "key_R2"
        ...
        if res['data']['timestamp'] == start_time:
            if type == "start":  # walking
                if 0 <= res['data']['status'] <= 2:
                    ...
                    return res  #else: # up  #    if 0 <= res['status'] <= 2:  #        await asyncio.sleep(1)  #    else:  #        return
